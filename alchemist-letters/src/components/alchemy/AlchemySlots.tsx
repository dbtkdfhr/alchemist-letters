    ...
    <div className="mt-6">
            ...
            >
              {ingredient ? (
                <div className="text-center">
                  <div className="font-handwriting text-sm text-ink">{ingredient.name}</div>
                  <div className="font-ui text-xs text-ink-light/50 mt-1">클릭하여 제거</div>
                </div>
              ) : (
                <span className="font-ui text-xs text-ink-light/30">비어있음</span>
              )}
            </div>
          )
        })}
      ...
      {lastResult && (
        <div className={`
          ...
          ${lastResult.result.grade === 'success' || lastResult.result.grade === 'perfect'
